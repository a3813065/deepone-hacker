...
def download_file(url, filename, placeholder='./placeholder.png'):
    """
    嘗試下載文件，若失敗則記錄錯誤並跳過下載。
    """
    print(f'嘗試下載 {filename}...')
    print('URL:', url)
    ...
    while download_times > 0:
        try:
            ...
            return
    # 如果完全下載失敗，創建一個占位文件
    print(f"下載 {filename} 最終失敗，使用占位圖片。")
    if not os.path.exists(placeholder):
        print(f"占位圖片 {placeholder} 不存在，請檢查！")
    else:
        # 複製占位圖片到目標路徑
        with open(placeholder, 'rb') as ph:
            with open(filename, 'wb') as dest:
                dest.write(ph.read())


# 加载预览图
def load_preview(json_list, placeholder='./placeholder.png'):
    """
    加載預覽圖，下載失敗時使用占位圖。
    """
    print("加載預覽圖……")
    ...
    for json_file in json_list:
        ...
        target_file = "episode/" + json_file.split("_")[0] + ".png"
        download_file(episode_url, target_file, placeholder=placeholder)
    print("加載預覽圖完成")
...
# 显示列表
def load_list(json_list, placeholder='./placeholder.png'):
    """
    顯示列表，若圖片不存在，使用占位圖片。
    """
    li_h = 100
    ...
    for li in json_list:
        ...
        img_path = "./episode/" + li.split('_')[0] + '.png'
        if not os.path.exists(img_path):
            img_path = placeholder  # 使用占位圖片
        li_button.set_img(img_path)
